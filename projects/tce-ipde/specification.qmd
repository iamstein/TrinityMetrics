...
-   The literature already reports the expected catch-up result, that
    ...
    without shortening the trial much. Present the **catch-up helps patients, but
    not time to evaluate active dose** hypothesis (Section 26) as a replication of
    a known finding rather than as a new claim.
...
Read the literature before Phase 1. It is five papers against a
four-phase implementation, and one of them can change the premise.
[References](references.qmd) carries the queue in reading order, with
the question to answer against each. Two of the five bear on this
section directly.

**Simon 1997 is the design ancestry, not the place to start.** Designs 2
to 4 there combine single-patient cohorts with intra-patient escalation,
which is the catch-up design and the accelerated cohorts above.
Elmeliegy 2024 is closer to this project's problem and covers dose
selection across the first eight approved TCEs.

Raising the starting dose is out of scope. A better MABEL attacks the
same gap from the other end, and it is a complementary strategy rather
than a competing one, but this project holds $D_{\rm start}$ fixed and
varies what happens above it.

#### What Simon 1997 establishes

Read and checked against the paper. Page numbers are from *J Natl Cancer
Inst* 1997;89(15):1138–1147; the [reading
notes](simon97-notes.qmd) carry the rest. Simon's designs are numbered 1
to 4 and his intrapatient options are lettered A and B, which are not
this document's designs A, B and C.

::: {.callout-important appearance="simple"}
**Fewer patients is not a shorter trial.** Simon's design 2, accelerated
single-patient cohorts with single dose steps, uses 24.4 patients on
average against 39.9 for the standard design (p.1141) and saves no time
at all. Design 1 needs slightly *fewer* cohorts, "because design 2
sometimes overshoots its target and requires more cohorts at
de-escalated levels" (p.1142).

**Time is bought with dose step size.** Designs 3 and 4 "show
substantial savings over designs 1 and 2 because of their use of double
dose steps during the initial stage of the trials" (p.1142). Cohort
count is the paper's duration proxy, valid "when there is an excess of
patients available for entry in the trial" (Fig. 3 caption, p.1143).

**Intrapatient escalation moves no clock.** Adding it to the standard
design "has no effect on the number of patients or number of cohorts
compared with 1A" (p.1143). Removing it from the accelerated designs has
"little or no effect on the number of patients or cohorts required"
(p.1143).

**What it does buy is undertreated patients.** Patients whose worst
toxicity was grade 0–1 fall from 23.3 to 19.3 when intrapatient
escalation is added to the standard design (p.1143), and from 6.5 to 3.9
when it is added to the accelerated double-step design (p.1144 and
p.1142).

**No published number exists for the time effect.** No figure or table
in the paper splits the intrapatient options apart; cohort counts appear
only as histograms for the four default designs (Fig. 3, p.1143). The
comparison is the phrase "little or no effect". A literature search
...
2.  The efficient conventional comparator is Simon's design 3 with
    option A: accelerated single-patient cohorts, double dose steps, no
...
3.  The dose escalation factor in Section 13 stops being a parameter and
    becomes the mechanism. Simon's accelerated ladder moves about 2×
    per double step once past the bottom of a modified Fibonacci scale,
    and about 3.3× at the first step; the scout ladder in Section 6
    holds ~3.3× throughout.

Two things in this project are probably already answered in the
accelerated titration literature. The **catch-up helps patients, but not
time to evaluate active dose** hypothesis in Section 26 is that literature's own reported
result. The shape of an
efficient conventional comparator, which the critical comparator issue
above leaves as a parameter, is what Simon's designs 2 to 4 specify.
Both should be cited rather than re-derived.

One thing is not answered in any of the five. Their escalation rules
stop on toxicity and the quantity sought is a tolerated dose. None
carries a pharmacodynamic stopping rule, a scout-and-backfill split, or
a requirement that a separate cohort confirm an interpretable regimen,
so none bears directly on $T_{3,\rm eval}$, on the confirmation penalty,
or on the prediction-error axis. Their driver is uncertainty about where
toxicity begins; this project's driver is uncertainty about where
activity begins, in a range believed to sit below the toxic one.

The simulation is not made redundant by any of this, but it is narrower
than it looks. What remains to be shown is the value of the
...
toxicity statistics. Future work (Section 27b) carries the argument for
deferring them
...
EHA 2020 e-poster (Section 28, references). Verify the transcription
against the
...
4.  **From cohort 9 the step-up prefix is fixed and only the target
    ...
    and 400 mg. That is the structure the backfill regimen in Section 7 assumes for the
