...
-   The literature already reports the expected catch-up result, that
    ...
    without shortening the trial much. Present the **catch-up helps
    patients, not the clock** hypothesis (Section 26) as a replication of
...
**Zhou 2025 can change the premise.** A modified MABEL approach reported
to justify a 10-fold higher first-in-human starting dose attacks the
same gap from the other end, by moving $D_{\rm start}$ up rather than
traversing the gap faster. A generally applicable version shrinks
$R_{\rm predicted-gap}$ in Section 12 and the value of IPDE with it. The
two strategies are not exclusive and the simulation carries both, since
a higher starting dose is a change to one parameter. Run the scenarios
with $D_{\rm start}$ raised and report how much of the advantage
survives.

Two things in this project are probably already answered in the
accelerated titration literature. The **catch-up helps patients, not the
clock** hypothesis in Section 26 is that literature's own reported
...
1.  **Big gap, big gain.** IPDE produces substantial acceleration when
    the true active dose is many escalation steps above the FIH starting
    dose.
2.  **Good prediction erases it.** The IPDE advantage decreases sharply
    when the active dose is accurately predicted.
3.  **Catch-up helps patients, not the clock.** Catch-up IPDE primarily
    benefits participants rather than trial calendar time. This replicates the accelerated-titration literature
...
(Section 12), which is the same conclusion as the **good prediction erases it**
hypothesis in Section 26
